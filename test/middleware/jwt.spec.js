...
describe('jwt middleware', function () {
  ...
  it('should authenticate token and return user object on request', function (done) {
    ...
    next = function (e) {
      if (e) {
        ...
        try {
          assert.equal(req.user.iss, 'urn:lanetix/auth');
          assert.equal(req.user.id, 1234);
          assert.equal(req.user.name, 'Cookie Monster');
          assert.equal(req.user.aud, 'urn:lanetix/api');
          done();
        } catch (err) {
          done(err);
        }
  ...
  it('should authenticate with authV2 when authV1 fails', function (done) {
    ...
    next = function (e) {
      if (e) {
        ...
        try {
          assert.equal(req.user.iss, 'https://lanetix.auth0.com');
          assert.equal(req.user.id, 5678);
          assert.equal(req.user.name, 'Elmo');
          assert.equal(req.user.aud, 'urn:lanetix/api');
          done();
        } catch (err) {
          done(err);
  ...
  it('should deny access with expired token', function (done) {
    ...
    next = function (e) {
      if (e) {
        try {
          should.not.exist(mock.req.user);
